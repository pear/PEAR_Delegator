 ...
 */
require_once 'Delegator/Internal/ForwardProxy.php';
 ...
 * }
 *
 * foo->addDelegate(new Delegate);  //add the delegate; see also Static Delegates
 ...
 * the calling delegator as a the first argument. Consquently, such a
 ...
 * not guaranteed to be of the form described above. Their signatures can be
 ...
 * </code>
 * 
 * Indeed, a mixed list of delegates can be called upon:
 * <code>
 ...
 * Hence, classes themselves (specified by classnames) can be delegates
 *
 ...
 * the PEAR_Delegator class recognizes this by viewing such delegates as <i>subdelegates</i>,
 ...
 *    //instead of $this->hasDelegate(), since the latter may return true
 *    //even though no delegate may be able to respond.
 *    if ($this->hasDelegate())
 ...
 * of delegation is for an object to delegate selected methods, calling its
 ...
 *    <b>Note</b>: Subdelegators cache their own delegates, so calls are passed down the hiearchy
 ...
 *                               of a delegate.
 * 4. <b>true delegate</b>     : A delegate that follows the $owner model.
 * 5. <b>false delegate</b>    : A delegate that does not follow the $owner model; any 
...
class PEAR_Delegator
{
    ...
    /**
     * An internal variable for turning on extensions.
	 * @var boolean
     */
     ...
     * 
     * This adds the extensions if they have been loaded.
     */
     ...
     */
     ...
     {
        if (!self::$_addExtensions) {
            require_once 'Delegator/Internal/Extension.php';
            self::$_addExtensions = true;
        }
        
        $args = func_get_args();
        foreach ($args as $delegator) {
            $delegator->addDelegate('PEAR_Delegator_Internal_Extension');
        }
     }
        ...
        foreach ($args as $delegate)
        {
            ...
            if (is_string($delegate)) {
                if (!class_exists($delegate)) {
                    require_once 'Delegator/Exception/DelegateUndefined.php';
                    
                    $exception = new PEAR_Delegator_Exception_DelegateUndefined($delegate);
     ...
     *      PEAR_Delegator::getDelegateExact(), PEAR_Delegator_Internal_Extension::getDelegateRecursive(),
    ...
    public function getDelegate($classname1)
    {
        ...
        foreach ($args as $classname) {                
            foreach ($this->_delegates as $delegate) {
                if (PEAR_Delegator::isA($delegate, $classname)) {
    ...
    public function getDelegateExact($classname1)
    {
        ...
        foreach ($args as $classname) {
            foreach ($this->_delegates as $delegate) {
                if (PEAR_Delegator::isAExact($delegate, $classname)) {
                    $result[$classname][] = $delegate;
                }
            }
        }
     ...
     * 
     * @see PEAR_Delegator_Internal_Extension::getDelegateForMethodRecursive(), PEAR_Delegator_Internal_Extension::getDelegateForMethodRecursiveExact(),
     *      PEAR_Delegator_Internal_Extension::getDelegateForMethodFirst(), PEAR_Delegator::methodExists()
     ...
     *     ...
     * )
     * Note: If a single method name is passed, a traditional array with numbered
     * elements is returned.
     * </pre>
     */
    ...
    public function hasDelegateExact($specifier = null)
    {
        ...
        foreach ($this->_delegates as $delegate) {
            if (PEAR_Delegator::isAExact($delegate, $specifier)) {
                return true;
            }
        }
     ...
     *
     * @see PEAR_Delegator::hasDelegateExact(), PEAR_Delegator::isA();
     ...
     *
     * @see PEAR_Delegator::removeDelegate(), PEAR_Delegator_Internal_Extension::removeDelegateRecursiveExact()
     */
   ...
   /**
     ...
     *                           delegate to remove.
     * @see PEAR_Delegator::removeAllDelegates(), PEAR_Delegator_Internal_Extension::removeDelegateRecursiveExact()
     ...
     */
    public static function isAExact($specifier, $classname)
    {
     ...
     * Determines if a class or instance object is of the given type.
     *
     * This method is an extension of the isAExact() method. It also
     ...
     * @return  bool    true if it is, false if it is not.
     */
    public static function isA($specifier, $classname)
    {
        if (PEAR_Delegator::isAExact($specifier, $classname)) {
            return true;
     ...
     *
     * This method is an extension of the method_exists function;
     ...
     * @return  bool    true if it is, false if it is not.
     */
    public static function methodExists($specifier, $method)
    {
        if (method_exists($specifier, $method)) {
            return true;
     ...
     * delegators.
     *
     * @see PEAR_Delegator::methodExists()
     * @uses PEAR_Delegator::methodExists()
    ...
    {
        return PEAR_Delegator::methodExists($this, $method);
    }
     ...
     * @throws PEAR_Delegator_Exception_MethodUndefined
     */
    protected function cacheMethod($method)
    {
        ...
        require_once 'Delegator/Exception/MethodUndefined.php';
        
        throw new PEAR_Delegator_Exception_MethodUndefined($method);
    }
    ...
    public function __call($method, $args = array(null))
    {
        ...
        if (!array_key_exists($method, $this->_method_map)) {
            ...
            } catch(PEAR_Delegator_Exception_MethodUndefined $exception) {
                die('<b>Fatal error</b>: ' . $exception->getMessage());
            }
        }
        
        $forwardingDelegator = (isset($args[0]) ? $args[0] : null);
                
        if (!(is_object($forwardingDelegator)) || !($forwardingDelegator instanceof PEAR_Delegator_Internal_ForwardProxy)) {
            $forwardingDelegator = new PEAR_Delegator_Internal_ForwardProxy($this);
            $args = array_merge(array($forwardingDelegator), $args);
        }
        
        $delegate       = $this->_method_map[$method];
                
        if (($delegate instanceof PEAR_Delegator) && !(method_exists($delegate, $method))) {
